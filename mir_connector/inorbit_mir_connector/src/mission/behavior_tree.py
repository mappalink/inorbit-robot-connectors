...
The tree for a single MissionStepExecuteMirNativeMission step:
    ...
    BehaviorTreeSequential("Navigate N waypoints")
      ├── CreateMirNativeMissionNode   → create_mission + N × add_action + queue
...
class SharedMemoryKeys(StrEnum):
    """Keys for MiR-specific shared memory entries."""
...
class MirBehaviorTreeBuilderContext(BehaviorTreeBuilderContext):
    """Extended context for MiR behavior tree construction.

    Carries the MiR API client, missions group ID, and firmware version
    needed by the behavior tree nodes.
    """
...
class CreateMirNativeMissionNode(BehaviorTree):
    ...
    Follows the same pattern as connector.send_waypoint_over_missions()
    but batches multiple waypoints into a single mission for continuous
    motion planning.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._connector_type = context.connector_type
    ...
    async def _execute(self):
        ...
        try:
            # 1. Create mission definition in the temporary missions group
            ...
            # 2. Add one move_to_position action per waypoint
            for i, wp in enumerate(waypoints):
                ...
                # (matches connector.send_waypoint_over_missions logic)
...
class WaitForMirMissionCompletionNode(BehaviorTree):
    """Polls the MiR mission queue until the queued native mission completes.

    Checks the mission queue for the mission definition GUID and monitors
    its state until Done, Abort, or Error.
    """
    ...
    async def _execute(self):
        ...
        while True:
            ...
            # Find our mission in the queue by definition GUID
            our_entry = None
            for entry in queue:
            ...
            if our_entry is None:
                # Mission not in queue — it may have been completed and cleaned up,
                # or aborted via DELETE /mission_queue. Either way, treat as done
                # (abort is handled by the error handler path).
            ...
            if state in (_STATE_ABORT,):
                error_msg = f"MiR mission {mission_guid} was aborted"
                logger.error(error_msg)
                self._shared_memory.set(SharedMemoryKeys.MIR_ERROR_MESSAGE, error_msg)
                raise RuntimeError(error_msg)

            # Still Pending or Executing — keep polling
...
class MirNodeFromStepBuilder(NodeFromStepBuilder):
    ...
    def visit_execute_mir_native_mission(
        ...
        """Build behavior tree for executing a compiled MiR native mission."""
        ...
        sequence.add_node(
            CreateMirNativeMissionNode(
                self._mir_context,
                step,
                label=f"Create MiR mission '{step.label}'",
            )
        )
